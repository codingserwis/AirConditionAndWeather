...
type Config = {domStrings: {
					...
					pm10Data: string,
					pm25Data: string,
					IJPDataContainer: string,
					lastUpdateContainer: string,
					cond: string,
					temp: string,
					fTemp: string,
					windDir: string,
	...
	const connectionStatus = (response: {status: number, statusText: string})=> {
	...
	// last update info 
	const lastUpdateInfo = (weatherData: {current: {last_updated: string}})=> {
	...
	const checkWindDirection = (weatherData: {current: {wind_dir: string}}): string => {
	...
	const checkCurrentWeatherCondition = (weatherData: {current: {is_day: number, condition: {code: number}}}): string => {
	...
	const insertIJPData = (data)=> {
		let html: string, 
			stationName, ijpBcgColor, pm25BcgColor, pm10BcgColor,
